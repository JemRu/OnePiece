...
class Onepiece extends CI_Controller {
	function __construct()
	{
		...
		include("http://2.ingonepiece.sinaapp.com/onepiece/qiniu/branches/master/qiniu/rs.php");
	}
...
//使用七牛的接口读取图片;
//		$key = $artwork_infor['artwork_url'];
//		$domain = 'phpsdk.qiniudn.com';
//		$accessKey = 'sSUYEVYfSUCprw28Qfu9OOqbIaqG87BVL1zEg1mD';
//		$secretKey = 'LqZNiPWE3F-vLFWSmNNlXHsvEZmx567sC4qmFHlc';
...
//		$privateUrl = $getPolicy->MakeRequest($baseUrl, null);
		...
		//找出该图片所有被划过的部分
		$clip_sql = "select * from record where artwork_id = $artwork_id ORDER BY clip_position";
...
//找出该图片所有被划过的部分
		$clip_sql = "select * from record where artwork_id = ".$artwork_infor['artwork_id']." ORDER BY clip_position";
		$clip_infor = $mysql->getData($clip_sql);
//为页面旁边的窗口做数据准备
//两种类别的图片id，url
//按照切割分数排序取前四个；
...
//裁剪区域的宽和高
			$width = $former_width/20;
			$height = $former_height/20;
//保存每张原图的宽和高
			$mysql = new SaeMysql();
...
//计算宽，高需要被切割多少份
			$x_clip_num = $former_width/$width;
			$y_clip_num = $former_height/$height;

//按照切割位置计算切割点坐标
			$y_multiples = $clip_position/20;
			$x_multiples = $clip_position%20;
			$x = $x_multiples*$width;
...
//插入分图信息
		$insert_clip_sql = "INSERT INTO  `app_ingonepiece`.`record` (
							...
							VALUES (
							NULL ,  '$artwork_id',  'zhou',  '$clip_after_draw',  '$clip_before_draw',  '$clip_position',  '$coordinate'
							)";
		$mysql = new SaeMysql();
